...
// mockKafka implements the minimal ProducerAPI used by OrderService
type mockKafka struct{
    publishedTopic string
    publishedMsg   []byte
}
func (m *mockKafka) Publish(topic string, message []byte) error {
...
func TestCreateOrder_PublishesToKafkaAndSNS(t *testing.T) {
    // Arrange
    kafka := &mockKafka{}
    sns := &mockSNS{}

    // Use a nil repo (we don't reach DB in CreateOrder)
    svc := NewOrderService(nil, kafka, "checkout.test", sns, "arn:aws:sns:eu-west-2:000000000000:order-events")
    ...
    if kafka.publishedTopic != "checkout.test" {
        t.Fatalf("expected kafka topic checkout.test got %s", kafka.publishedTopic)
    }
    if len(kafka.publishedMsg) == 0 {
        t.Fatalf("expected kafka message to be published")
    }
